...
async def log_validator_message(
    ...
    *,
    ...
    is_validator: bool | None = None,  # kept for API compatibility, no longer needed
    ...
    try:
        if direction == "request" and signature and nonce and signer_ss58:
            # Single round-trip: upsert requests, resolve validator/miner FKs by ss58,
            ...
            await session.execute(
                text("""
                    ...
                    SELECT
                        ...
                        :nonce,
                        :signer_ss58,
                        (SELECT id FROM validators WHERE ss58 = :signer_ss58),
                    ...
                    ON CONFLICT (request_fk) DO UPDATE SET
                        ...
                        signer_validator_fk = EXCLUDED.signer_validator_fk,
        ...
        logger.exception("validator_log_write_failed")

            ...
            if signature and nonce and signer_ss58:
                ...
                if is_validator:
                    validator_result = await session.execute(
                        select(Validator).where(Validator.ss58 == signer_ss58)
                    )
                    validator = validator_result.scalars().first()
                ...
                if signed_entry is None:
                    signed_entry = SignedRequest(
                        ...
                        signature=signature,
                        nonce=nonce,
                        signer_validator_fk=signer_validator_fk,
                    ...
                    signed_entry.signer_validator_fk = signer_validator_fk
